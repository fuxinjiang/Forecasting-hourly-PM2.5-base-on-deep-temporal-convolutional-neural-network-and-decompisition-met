#主要用ceemdan分解之后的数据结合模型，进行pm2.5预测
#在tcn_torch_horizon1中加入分解的数据，看一下效果
# coding=utf-8
# /usr/bin/env python3
'''
Author:Fuxin Jiang
...
def train(model, data_iter, val_x, val_time_embedding, val_weather_embedding, val_y, optimizer, num_epochs, device, scaler, horizon):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        if val_mape_cpu<val_l:
            torch.save(model, 'model/ceemdan_tcn_model_%s.pkl' %(str(horizon)))
...
if __name__ == "__main__":
    ...
    model = torch.load('model/ceemdan_tcn_model_%s.pkl' %(str(horizon)))
    ...
    data_frame.to_excel("ceemdan_tcn_horizon_%d.xlsx" %horizon, index=False)
