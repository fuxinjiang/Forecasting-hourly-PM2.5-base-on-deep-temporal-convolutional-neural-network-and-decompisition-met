...
def data_main(df, chosen_scale, lag, horizon):
    ...
    #x_con = np.array(data[['pm2.5', 'Imf1','Imf2','Imf3','Imf4','Imf5','Imf6','Imf7','Imf8','Imf9','Imf10','Imf11', 'Imf12', 'Imf13', 'Imf14','Imf15', 'Residual']])
    #x_con = np.array(data[['pm2.5']])
...
def SupportVectorRegression(C, gamma, Train_X, Train_Y):
    lin_clf = SVR(kernel='linear', epsilon=0.01, C=C, gamma=gamma)
...
if __name__ == "__main__":
    ...
    for horizon in range(1,4):
        #test
        scale, x_matrix, y_matrix, test_length = data_main(data, 'StandardScaler', lag, horizon)
        x_matrix_train = x_matrix[15000:-test_length, :]
        x_matrix_test = x_matrix[-test_length:, :]
        y_matrix_train = y_matrix[15000:-test_length, :]
        y_matrix_test = y_matrix[-test_length:, :]

        #计算线性回归的
        print("开始svr的计算")
        K = 5
        C_range = np.array([63.0957344480193])
        gamma_range = np.array([0.01])
        #C_range = np.logspace(-3, 3, 11)
        #gamma_range = np.logspace(-2, 2, 11)
        SVR_ = SupportVectorRegression(10,0.01, x_matrix_train, y_matrix_train.reshape(-1,)) 
        #SVR_ = KoldValidation_SupportVectorMachine(C_range, gamma_range, K, x_matrix_train, y_matrix_train.reshape(-1,)) 
        
        svr_pred_y = SVR_.predict(x_matrix_test).reshape((-1, 1))
        svr_pred_y = scale.inverse_transform(svr_pred_y).reshape((-1, 1))
        svr_true_y = scale.inverse_transform(y_matrix_test)
        data_frame = pd.DataFrame({})
        data_frame['real value'] =svr_true_y.reshape(1, -1)[0]
        data_frame['pred value'] = svr_pred_y.reshape(1, -1)[0]
        data_frame = pd.DataFrame(data_frame)
        data_frame.to_excel("ceemdan_svr_horizon_%d_.xlsx" %horizon, index=False)
        # 开始计算误差
        print(test_mse(svr_true_y, svr_pred_y))
        print(test_mae(svr_true_y, svr_pred_y))
        print(test_mape(svr_true_y, svr_pred_y))
    """
    result1 = pd.read_excel("ceemdan_svr_horizon_1_.xlsx")
    result2 = pd.read_excel("ceemdan_svr_horizon_2_.xlsx")
    result3 = pd.read_excel("ceemdan_svr_horizon_3_.xlsx")
