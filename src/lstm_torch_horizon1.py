...
class LSTMModel(nn.Module):
    def __init__(self, input_size, nhour_embedding_size, wday_embedding_size, season_embedding_size, weather_embedding_size, output_size, hidden_size, num_layer, dropout=0.0):
        ...
        self.lstm = nn.LSTM(input_size+nhour_embedding_size+wday_embedding_size+season_embedding_size+weather_embedding_size, hidden_size, num_layers=num_layer)
...
def train(model, data_iter, val_x, val_time_embedding, val_weather_embedding, val_y, optimizer, num_epochs, device,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if val_mape_cpu < val_l:
            torch.save(model, 'model/lstm_model_%s.pkl' % (str(horizon)))
...
if __name__ == "__main__":
    ...
    num_epochs = 200
    num_layer = 1
    model = LSTMModel(input_size, nhour_embedding_size, wday_embedding_size, season_embedding_size,
    ...
    model = torch.load('model/lstm_model_%s.pkl' % (str(horizon)))
    with torch.no_grad():
        model.to(device)
        model.eval()
    ...
    data_frame.to_excel("lstm_horizon_%d.xlsx" %horizon, index=False)
