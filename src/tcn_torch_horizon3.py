...
def data_main(df, chosen_scale, lag, horizon, test_length):
    data = df.copy()
    #首先确定测试集的长度
    x_con = np.array(data[['pm2.5', 'pm10', 'so2', 'no2', 'o3', 'co', '温度', '湿度', '降雨量', 'wind_x', 'wind_y', '大气压']])
    ...
    x_matrix = np.zeros((length, lag, 12))
...
class TcnModel(nn.Module):
    def __init__(self, input_size, nhour_embedding_size, wday_embedding_size, season_embedding_size, weather_embedding_size, output_size, num_channels, kernel_size=2, dropout=0.2):
        super(TcnModel, self).__init__()

        self.tcn = TemporalConvNet(input_size+nhour_embedding_size+wday_embedding_size+season_embedding_size+weather_embedding_size, num_channels, kernel_size, dropout)
        self.output_dense = nn.Linear(num_channels[-1]+nhour_embedding_size+wday_embedding_size+season_embedding_size, output_size)
...
def train(model, data_iter, val_x, val_time_embedding, val_weather_embedding, val_y, optimizer, num_epochs, device, scaler, horizon):
    model.to(device)
    ...
    for epoch in range(1, num_epochs+1):
        ...
        if val_mape_cpu<val_l:
            torch.save(model, 'model/tcn_model_%s.pkl' %(str(horizon)))
        ...
        #对模型进行保存
        print("train_loss:%f | val_loss:%f| val_mse:%f| val_mae:%f| val_mape:%f| time: %f s" %(l_sum/batch_num, l_val_sum, val_mse, val_mae, val_mape, time_final-time_start))
...
if __name__ == "__main__":
    ...
    lag = 24
    horizon = 3
    scale, x_matrix, y_matrix, time_embedding, weather_embedding = data_main(Data, 'StandardScaler', lag, horizon, test_lenth)
    ...
    input_channels = x_matrix_train.shape[2]
    ...
    channel_sizes = [32, 32, 16, 16]
    kernel_size = 2
    dropout = 0
    batch_size = 128
    num_epochs = 200
    model = TcnModel(input_channels, nhour_embedding_size, wday_embedding_size, season_embedding_size, weather_embedding_size, output_size, channel_sizes, kernel_size=kernel_size, dropout=dropout)
    ...
    '''
    train_loss, val_loss, model = train(model, train_iter, x_test, time_embedding_test, weather_embedding_test, y_test, optimizer, num_epochs, device, scale, horizon)
    '''
    mae_loss = Mae_loss(scale)
    mse_loss = Mse_loss(scale)
    mape_loss = Mape_loss(scale)
    
    model = torch.load('model/tcn_model_%s.pkl' %(str(horizon)))
    ...
    data_frame.to_excel("tcn_horizon_%d.xlsx" %horizon, index=False)
