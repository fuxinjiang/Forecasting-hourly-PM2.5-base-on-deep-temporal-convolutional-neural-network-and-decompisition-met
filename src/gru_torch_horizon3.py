...
def train(model, data_iter, val_x, val_time_embedding, val_weather_embedding, val_y, optimizer, num_epochs, device,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if val_mape_cpu < val_l:
            torch.save(model, 'model/gru_model_%s.pkl' % (str(horizon)))
...
if __name__ == "__main__":
    ...
    model = torch.load('model/gru_model_%s.pkl' % (str(horizon)))
    with torch.no_grad():
        model.to(device)
        model.eval()
    ...
    data_frame.to_excel("gru_horizon_%d.xlsx" %horizon, index=False)
