...
def data_main(df, chosen_scale, lag, horizon):
    ...
    x_con = np.array(data[['pm2.5', 'pm10', 'so2', 'no2', 'o3', 'co', '温度', '湿度', '降雨量', 'wind_x', 'wind_y','大气压', 'hour', 'dayofweek', 'month']])
    ...
    x_matrix = np.zeros((length, lag * 15))
...
if __name__ == "__main__":
    ...
    for horizon in range(1,4):
        ...
        svr_pred_y = scale.inverse_transform(reg_pred_y).reshape((-1, 1))
        ...
        data_frame.to_excel("svr_horizon_%d.xlsx" %horizon, index=False)
        # 开始计算误差
    ...
    result1 = pd.read_excel("svr_horizon_1.xlsx")
    result2 = pd.read_excel("svr_horizon_2.xlsx")
    result3 = pd.read_excel("svr_horizon_3.xlsx")
